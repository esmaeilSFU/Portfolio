...
let dcodeweaverElement = document
  .getElementById("D-CodeWeaver")
  .querySelector("button");
dcodeweaverElement.addEventListener("click", (event) => {
  window.open("DCodeWeaver/index.html", "_blank");
});
...
// Open links on click
let maxprofixElement = document
  .getElementById("maxprofix")
  .querySelector("button");
maxprofixElement.addEventListener("click", (event) => {
  window.open("https://www.maxprofix.ca/", "_blank");
});

let flavourfeastElement = document
  .getElementById("flavourfeast")
  .querySelector("button");
flavourfeastElement.addEventListener("click", (event) => {
  window.open("https://www.flavourfeast.ca/", "_blank");
});

let perfwallElement = document
  .getElementById("perfwall")
  .querySelector("button");
perfwallElement.addEventListener("click", (event) => {
  window.open("https://esmaeilsfu.github.io/Perfwall/", "_blank");
});

let conCreateElement = document
  .getElementById("con-create")
  .querySelector("button");
conCreateElement.addEventListener("click", (event) => {
  window.open("ConCreate/index.html", "_blank");
});

let diamondwallElement = document
  .getElementById("dimond-wall")
  .querySelector("button");
diamondwallElement.addEventListener("click", (event) => {
  window.open("Diamondwall/index.html", "_blank");
});

let twistedArcElement = document
  .getElementById("twisted-arc")
  .querySelector("button");
twistedArcElement.addEventListener("click", (event) => {
  window.open("TwistedArc/index.html", "_blank");
});

let steelDomeElement = document
  .getElementById("steel-dome")
  .querySelector("button");
steelDomeElement.addEventListener("click", (event) => {
  window.open("SteelDome/index.html", "_blank");
});

let homeAccessories = document
  .getElementById("home-accessories")
  .querySelector("button");
homeAccessories.addEventListener("click", (event) => {
  window.open("HomeAccessories/index.html", "_blank");
});

let toothbrush = document.getElementById("toothbrush").querySelector("button");
toothbrush.addEventListener("click", (event) => {
  window.open("ToothBrush/index.html", "_blank");
});

let soundWall = document.getElementById("sound-wall").querySelector("button");
soundWall.addEventListener("click", (event) => {
  window.open("SoundWall/index.html", "_blank");
});

let stickerDesign = document
  .getElementById("freezer-sticker")
  .querySelector("button");
stickerDesign.addEventListener("click", (event) => {
  window.open("Sticker/index.html", "_blank");
});

let renaIntrocut = document
  .getElementById("renaIntrocut")
  .querySelector("button");
renaIntrocut.addEventListener("click", (event) => {
  window.open("RenaIntrocut/index.html", "_blank");
});
...
navigatorContainer.style.backgroundColor = "transparent";
...
for (let i = 0; i < allCategories.length; i++) {
  ...
  pTag.className = "Navigator-innerText";
  ...
  workItemCount += allCategories[i].children.length;
  element.style.flex = `${allCategories[i].children.length}`;
  element.addEventListener("click", () => {
    let categoryOffsetY =
      allCategories[i].getBoundingClientRect().top + window.scrollY;
    window.scrollTo({
      top: categoryOffsetY - 90,
      behavior: "smooth",
    });
  });
}
...
// slider.style.backgroundColor = "rgba(107, 107, 107,0)";
...
// icon.style.transform = "translate( 100px, -8px)";
...
document.addEventListener("scroll", () => {
  ...
  for (i = 0; i < allCategories.length; i++) {
    let ItemTopPos =
      allCategories[i].getBoundingClientRect().top + window.scrollY;
    let ItemBotPos =
      allCategories[i].getBoundingClientRect().top +
      allCategories[i].getBoundingClientRect().height +
      window.scrollY;

    if (window.scrollY > ItemTopPos - 150 && window.scrollY < ItemBotPos) {
      navigatorContainer.children[i].style.opacity = "100%";
      navigatorContainer.children[i].style.transform =
        "scaleX(110%) translateX(5%)";
    } else {
      navigatorContainer.children[i].style.opacity = "60%";
      navigatorContainer.children[i].style.transform =
        "scaleX(100%) translateX(0%)";
    }
  }
...
ServiceContainer.style.height = `${
  window.innerHeight - footerElement.offsetHeight
}px`;
let currentLocation = [];
let winElement;
Array.from(ServiceContainer.getElementsByTagName("li")).forEach((a) => {
  ...
  a.addEventListener("mouseenter", (e) => {
    ...
    winElement.style.width = `${ServiceContainer.offsetWidth / 2}px`;
    ...
    winElement.children[0].children[0].style.display = "none";
...
upwardArrowIcon.style.fontSize = "1.5rem";
upwardArrowIcon.style.opacity = 0.25;
upwardArrowIcon.style.position = "fixed";
upwardArrowIcon.style.top = `${window.innerHeight - 30}px`;
